	...
	C: Circuit<E::Fr>,
{
	...
	Ok(Parameters {
		...
		a: Arc::new(a.into_iter().filter(|e| bool::from(!e.is_identity())).collect()),
		b_g1: Arc::new(b_g1.into_iter().filter(|e| bool::from(!e.is_identity())).collect()),
		b_g2: Arc::new(b_g2.into_iter().filter(|e| bool::from(!e.is_identity())).collect()),
	})
}
