...
impl UInt32 {
		...
		CS: ConstraintSystem<Scalar>,
	{
		...
		let bits = values
			...
			.map(|(i, v)| {
				...
				write!(&mut buf, "allocated bit {}", i).unwrap();
				let s = buf.as_str();
	...
	pub fn from_bits_be(bits: &[Boolean]) -> Self {
		...
		UInt32 { value, bits: bits.iter().rev().cloned().collect() }
	}
	...
	pub fn rotr(&self, by: usize) -> Self {
		...
		let new_bits =
			self.bits.iter().skip(by).chain(self.bits.iter()).take(32).cloned().collect();

		UInt32 { bits: new_bits, value: self.value.map(|v| v.rotate_right(by as u32)) }
	}
	...
	pub fn shr(&self, by: usize) -> Self {
		...
		UInt32 { bits: new_bits, value: self.value.map(|v| v >> by as u32) }
	}
		...
		CS: ConstraintSystem<Scalar>,
	{
		Self::triop(
			...
			|cs, i, a, b, c| {
				let mut buf = ArrayString::<32>::new();
				write!(&mut buf, "maj {}", i).unwrap();
				let s = buf.as_str();

				Boolean::sha256_maj(cs.namespace(|| s), a, b, c)
			},
		)
	}
		...
		CS: ConstraintSystem<Scalar>,
	{
		Self::triop(
			...
			|cs, i, a, b, c| {
				let mut buf = ArrayString::<32>::new();
				write!(&mut buf, "ch {}", i).unwrap();
				let s = buf.as_str();

				Boolean::sha256_ch(cs.namespace(|| s), a, b, c)
			},
		)
	}
		...
		CS: ConstraintSystem<Scalar>,
	{
		...
		let bits = self
			...
			.map(|(i, (a, b))| {
				let mut buf = ArrayString::<32>::new();
				write!(&mut buf, "xor of bit {}", i).unwrap();
				let s = buf.as_str();

				Boolean::xor(cs.namespace(|| s), a, b)
			})
			.collect::<Result<_, _>>()?;
	...
	{
		// Make some arbitrarufmt = "0.2.0"y bounds for ourselves to avoid overflows
		// in the scalar field
		...
		while max_value != 0 {
			let mut buf = ArrayString::<32>::new();
			write!(&mut buf, "result bit {}", i).unwrap();
			let s = buf.as_str();
...
mod test {
	...
	fn test_uint32_from_bits_be() {
		...
		for _ in 0..1000 {
			let v = (0..32).map(|_| Boolean::constant(rng.next_u32() % 2 != 0)).collect::<Vec<_>>();
