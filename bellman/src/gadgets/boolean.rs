...
pub fn u64_into_boolean_vec_le<Scalar: PrimeField, CS: ConstraintSystem<Scalar>>(
	...
	let bits = values
		...
		.map(|(i, b)| {
			...
			let s = buf.as_str();
			Ok(Boolean::from(AllocatedBit::alloc(cs.namespace(|| s), b)?))
		})
...
>(
	...
	let bits = values
		...
		.map(|(i, b)| {
			...
			write!(&mut buf, "bit {}", i).unwrap();
			let s = buf.as_str();
			AllocatedBit::alloc(cs.namespace(|| s), b)
		})
		.collect::<Result<Vec<_>, SynthesisError>>()?;
	...
	fn test_xor() {
		for a_val in [false, true].iter() {
			for b_val in [false, true].iter() {
				...
				assert!(
					cs.get("xor result") == if *a_val ^ *b_val { Field::ONE } else { Field::ZERO }
				);
	...
	fn test_and() {
		for a_val in [false, true].iter() {
			for b_val in [false, true].iter() {
				...
				assert!(cs.get("b/boolean") == if *b_val { Field::ONE } else { Field::ZERO });
				assert!(
					cs.get("and result") == if *a_val & *b_val { Field::ONE } else { Field::ZERO }
				);
