	...
	*/
...

	{
		...
		for i in 0..10 {
			...
			write!(&mut buf, "round {}", i).unwrap();
			let s = buf.as_str();
			let mut cs = cs.namespace(|| s);
	...
	for i in 0..8 {
		let mut buf = ArrayString::<32>::new();
		write!(&mut buf, "h[{i}] ^ v[{i}] ^ v[{i} + 8]", i = i).unwrap();
		let s = buf.as_str();
		let mut cs = cs.namespace(|| s);
...
// 				let s = buf.as_str();
// 				AllocatedBit::alloc(cs.namespace(|| s), Some(true)).unwrap().into()
// 			})
// 			.collect();
...
// 			0xbc, 0xe5,
// 		]);
// 		let input_bits: Vec<_> =
// 			(0..512).map(|_| Boolean::constant(rng.next_u32() % 2 != 0)).collect();
// 		blake2s(&mut cs, &input_bits, b"12345678").unwrap();
// 		assert_eq!(cs.num_constraints(), 0);
// 	}
