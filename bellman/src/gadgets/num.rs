...
impl<Scalar: PrimeField> Clone for AllocatedNum<Scalar> {
	fn clone(&self) -> Self {
		AllocatedNum { value: self.value, variable: self.variable }
	}
}
...
impl<Scalar: PrimeField> AllocatedNum<Scalar> {
		...
		CS: ConstraintSystem<Scalar>,
	{
			...
			CS: ConstraintSystem<Scalar>,
		{
			...
			for (i, v) in v.iter().enumerate() {
				if cur.is_none() {
					...
					write!(&mut buf, "and {}", i).unwrap();
					let s = buf.as_str();
					cur = Some(AllocatedBit::and(cs.namespace(|| s), cur.as_ref().unwrap(), v)?);
				}
			}
		...
		for b in b.iter().by_vals().rev() {
			...
			if b {
				...
				let a_bit = AllocatedBit::alloc(cs.namespace(|| s), a_bit)?;
				...
				if !current_run.is_empty() {
					...
					let mut buf = ArrayString::<32>::new();
					write!(&mut buf, "run ending at {}", i).unwrap();
					let s = buf.as_str();
		...
		CS: ConstraintSystem<Scalar>,
	{
		...
		Ok(AllocatedNum { value, variable: var })
	}
...
impl<Scalar: PrimeField> From<AllocatedNum<Scalar>> for Num<Scalar> {
	fn from(num: AllocatedNum<Scalar>) -> Num<Scalar> {
		Num { value: num.value, lc: LinearCombination::<Scalar>::zero() + num.variable }
	}
}

impl<Scalar: PrimeField> Num<Scalar> {
	pub fn zero() -> Self {
		Num { value: Some(Scalar::ZERO), lc: LinearCombination::zero() }
	}
	...
	pub fn add_bool_with_coeff(self, one: Variable, bit: &Boolean, coeff: Scalar) -> Self {
		...
		Num { value: newval, lc: self.lc + &bit.lc(one, coeff) }
	}
}
...
mod test {
	...
	fn test_into_bits() {
		...
		for i in 0..200 {
			...
			for (b, a) in r.to_le_bits().iter().by_vals().rev().skip(1).zip(bits.iter().rev()) {
			...
			for i in 0..Scalar::NUM_BITS {
				let mut buf = ArrayString::<32>::new();
				write!(&mut buf, "bit {}/boolean", i).unwrap();
				let s = buf.as_str();
				let name = s;
				let cur = cs.get(&name);
