	...
	CS: ConstraintSystem<Scalar>,
{
	...
	Ok(sha256_compression_function(&mut cs, input, &get_sha256_iv())?
		.into_iter()
	...
	CS: ConstraintSystem<Scalar>,
{
	...
	for (i, block) in padded.chunks(512).enumerate() {
		...
		write!(&mut buf, "block {}", i).unwrap();
		let s = buf.as_str();
		cur = sha256_compression_function(cs.namespace(|| s), block, &cur)?;
	}
	...
	CS: ConstraintSystem<Scalar>,
{
	...
	let mut w = input.chunks(32).map(UInt32::from_bits_be).collect::<Vec<_>>();
	...
	for i in 16..64 {
		let mut buf = ArrayString::<32>::new();
		write!(&mut buf, "w extension {}", i).unwrap();
		let s = buf.as_str();

		let cs = &mut cs.namespace(|| s);
	...
	for i in 0..64 {
		let mut buf = ArrayString::<32>::new();
		write!(&mut buf, "compression round {}", i).unwrap();
		let s = buf.as_str();
	...
	*/
	...
	let h0 =
		a.compute(cs.namespace(|| "deferred h0 computation"), &[current_hash_value[0].clone()])?;

	let h1 = UInt32::addmany(cs.namespace(|| "new h1"), &[current_hash_value[1].clone(), b])?;

	let h2 = UInt32::addmany(cs.namespace(|| "new h2"), &[current_hash_value[2].clone(), c])?;

	let h3 = UInt32::addmany(cs.namespace(|| "new h3"), &[current_hash_value[3].clone(), d])?;

	let h4 =
		e.compute(cs.namespace(|| "deferred h4 computation"), &[current_hash_value[4].clone()])?;

	let h5 = UInt32::addmany(cs.namespace(|| "new h5"), &[current_hash_value[5].clone(), f])?;

	let h6 = UInt32::addmany(cs.namespace(|| "new h6"), &[current_hash_value[6].clone(), g])?;

	let h7 = UInt32::addmany(cs.namespace(|| "new h7"), &[current_hash_value[7].clone(), h])?;
...
mod test {
	...
	fn test_full_block() {
		...
		let input_bits: Vec<_> = (0..512)
			.map(|i| {
				let mut buf = ArrayString::<32>::new();
				write!(&mut buf, "input bit {}", i).unwrap();
				let s = buf.as_str();

				Boolean::from(
					AllocatedBit::alloc(cs.namespace(|| s), Some(rng.next_u32() % 2 != 0)).unwrap(),
				)
			})
			.collect();
		...
		for input_len in (0..32).chain((32..256).filter(|a| a % 8 == 0)) {
			...
			for (byte_i, input_byte) in data.into_iter().enumerate() {
				for bit_i in (0..8).rev() {
					let mut buf = ArrayString::<32>::new();
					write!(&mut buf, "input bit {} {}", byte_i, bit_i).unwrap();
					let s = buf.as_str();

					let cs = cs.namespace(|| s);
