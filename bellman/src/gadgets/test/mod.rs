...
fn proc_lc<Scalar: PrimeField>(terms: &[(Variable, Scalar)]) -> BTreeMap<OrderedVariable, Scalar> {
	...
	for &(var, coeff) in terms {
		map.entry(OrderedVariable(var)).or_insert(Scalar::ZERO).add_assign(&coeff);
	}
...
impl<Scalar: PrimeField> TestConstraintSystem<Scalar> {
	pub fn new() -> TestConstraintSystem<Scalar> {
		...
		map.insert("ONE".into(), NamedObject::Var(TestConstraintSystem::<Scalar>::one()));
	...
	pub fn set(&mut self, path: &str, to: Scalar) {
		match self.named_objects.get(path) {
			...
			Some(e) => {
				panic!("tried to set path `{}` to value, but `{:?}` already exists there.", path, e)
			},
...
fn test_cs() {
	...
	let a = cs.namespace(|| "a").alloc(|| "var", || Ok(Scalar::from(10))).unwrap();
	let b = cs.namespace(|| "b").alloc(|| "var", || Ok(Scalar::from(4))).unwrap();
